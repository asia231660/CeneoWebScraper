    ...
    if request.method =="POST":
        ...
        if response.status_code == requests.codes['ok']:
            ...
            if opinions_count:
                ...
                plt.title(f"Score histogram for {product_name} product")
                plt.savefig(f"app/static/charts/{product_id}_recomendation.png")
                plt.close()
...
def product(product_id):
    ...
    return render_template("product.html", product_id=product_id, opinions = opinions.to_html(classes=" table table-hover table-strped" , table_id= "opinions", index=False))
